...
async def lifespan(app: FastAPI):
    ...
    for row in rows:
        tid = row["task_id"]
        manager.task_ids.append(tid)
        manager.job_ids[tid] = json.loads(row["job_ids"])
    logging.info(f"Restored {len(rows)} tasks from database.")
...
def get_job_output(
    ...
    _verify_ownership(task_id, username)
    # verify the job belongs to this task
    conn = get_db()
