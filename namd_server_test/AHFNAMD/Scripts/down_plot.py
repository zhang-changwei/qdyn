...
if not os.path.exists('wdo.txt'):
    w1= np.loadtxt('pyband_do_weights_component_1.dat')
    w2= np.loadtxt('pyband_do_weights_component_2.dat')
    w = w1/(w1+w2)
    w = np.nan_to_num(w)
    np.savetxt('wdo.txt', w)

dat = np.loadtxt('pyband_do.dat')
Wht = np.loadtxt('wdo.txt')
nband = dat.shape[1] - 1
x = dat[:,0]
fig = plt.figure()
...
plt.savefig('pbanddo.png', dpi=360)
