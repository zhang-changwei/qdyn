...
if not os.path.exists('wup.txt'):
    w1= np.loadtxt('pyband_up_weights_component_1.dat')
    w2= np.loadtxt('pyband_up_weights_component_2.dat')
    w = w1/(w1+w2)
    w = np.nan_to_num(w)
    np.savetxt('wup.txt', w)

dat = np.loadtxt('pyband_up.dat')
Wht = np.loadtxt('wup.txt')
nband = dat.shape[1] - 1
x = dat[:,0]
fig = plt.figure()
...
plt.savefig('pbandup.png', dpi=360)
